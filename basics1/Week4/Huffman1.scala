...
object Huffman {
  ...
  // Part 1: Basics
    def weight(tree: CodeTree): Int = tree match {
      case l@Leaf(_, _) => l.weight
      case f@Fork(_, _, _, _) => f.weight
    }
  
    def chars(tree: CodeTree): List[Char] =  tree match {
      case l@Leaf(_, _) => List(l.char)
      case f@Fork(_, _, _, _) => f.chars
    }
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      chars.foldLeft(Map.empty[Char,Int])((m: Map[Char,Int], c: Char) => {
        m get c match {
          case None => m updated(c, 1)
          case Some(s) => m updated(c, s + 1)
        }
      }).toList
    }
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      freqs.foldLeft(List.empty[Leaf])(
          (result: List[Leaf], tuple: (Char, Int)) => Leaf(tuple._1, tuple._2) ::result
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = {
      if (trees.isEmpty) trees
        ...
        val (less, more) = newTrees.partition(t => weight(t) < fork.weight)
        less ::: fork :: more
      }
    }
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def loop(t: CodeTree, bs: List[Bit]): (Char, List[Bit]) =
        t match {
          case Fork(left, right, _, _) =>
            bs match {
              case 0 :: xs => loop(left, xs)
              case 1 :: xs => loop(right, xs)
              case _ => throw new Error("no more bits")
            }
          case Leaf(c, _) => (c, bs)
        }
      loop(tree, bits) match {
        case (c, Nil) => List(c)
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def loop(v: CodeTree, c: Char): List[Bit] = v match {
        case Leaf(_, _) => Nil
        case Fork(left, right, _, _) =>
          if (chars(left) contains c) 0 :: loop(left, c)
          else 1 :: loop(right, c)
      }
      text.flatMap(loop(tree, _))
    }
